...
> 마지막 업데이트: 2026-04-06 KST (세션 #18)

## 전체 진행률: 42/42 + Phase 4 완료 + Post-MVP 40건 완료 (1건 진행 중)
...
## 🎨 Phase 4: 디자인 개선 (10/10) ✅

- [x] 4-1. 홈 페이지 책 소개 프로모션 배너 추가 (표지 이미지 + 소개 문구 + "자세히 보기" 버튼)
- [x] 4-2. 네비게이션 바 구조 개편 (시뮬레이터 3개 드롭다운 그룹화 + 설명 텍스트)
- [x] 4-3. Navbar DOM 위치 수정 (#navbar-container를 #app 바깥으로 이동 — Vue 마운트 덮어씌움 문제)
- [x] 4-4. 저자명 업데이트 (책 표지 SVG + book.html에 "김대중 · 최근민" 반영)
- [x] 4-5. 디자인 가이드 문서 생성 (`.claude/rules/design-guide.md`)
- [x] 4-6. CSS 디자인 시스템 v3 적용 (CSS 변수 + 표준 컴포넌트 6종)
- [x] 4-7. 6개 페이지 디자인 표준 적용 (홈/타이밍/비용/퇴직연금/랭킹/인사이트)
- [x] 4-8. 네비게이션 바 리디자인 (다크 그라데이션 + 브랜드 아이콘 + 드롭다운 레이아웃)
- [x] 4-9. 차트 렌더링 수정 — canvas를 getElementById → Vue $refs로 변경 (비용/퇴직연금/ETF상세)
- [x] 4-10. 타이밍 시뮬레이터 백엔드 수정 + UX 개선 (MAX_YEAR 스코프 이동, 슬라이더 자동 시뮬레이션)
...
> MVP 이후 추가되는 기능/변경 사항은 별도 문서에서 관리합니다.
> **→ [`.claude/post-mvp.md`](post-mvp.md)** 참조
> 안정화 후 이 문서에 Phase 5+로 통합 예정
