...
### 추가 작업 — SPLG 가격 미갱신 원인 규명 및 수정

**증상**: 랭킹에서 특정 종목의 최종 데이터가 최근 종가가 아님 (사용자 제보)

**진단 경과**
1. 랭킹 30종목 전수 확인 → 29종목은 2026-09-02(최신 거래일) 정상, **SPLG만 2026-07-17에서 멈춤**
2. D1 전수 조회 → 갱신 지연 종목은 SPLG 단 하나
3. Yahoo chart API 직접 조회 → SPLG는 에러 없이 `close` 전부 `null` 반환.
   `firstTradeDate`가 2026-07-17로 리셋, `regularMarketTime`은 **2025-10-30**에 정지
4. **원인 확정**: SPDR Portfolio S&P 500 ETF가 **2025-10-31자로 티커를 SPLG → SPYM으로 변경**.
   `regularMarketTime`이 변경 직전 거래일과 정확히 일치 (사용자가 티커 변경 가능성을 먼저 제기)

**왜 7주간 드러나지 않았나**
- `DailyUpdateService._updateTicker()`가 0건 반환 시 `skipped`로 분류 — 실패가 아닌 정상 취급
- `PriceService._fetchRecent()`는 빈 배열이면 조용히 기존 D1 데이터 반환
- 에러가 발생하지 않아 로그에도 남지 않고, 화면에는 옛 데이터가 최신처럼 표시됨

**피해**: 랭킹 1Y 수익률 15.52% / 초과수익 -3.34 / 21위로 왜곡
(동일 지수 추종 VOO 18.88%, +0.02, 12위)

**수정 전 검증**
- SPYM 가격이 이미 D1에 5,230건(2005-11-15~2026-09-02) 최신 상태로 존재
- SPLG의 모든 날짜가 SPYM에 존재 (누락 0건) → 삭제해도 데이터 손실 없음
- 2020년 이후 종가 완전 일치. 2009~2019 구간 845건은 소수점 셋째 자리 차이
  (Yahoo 조정종가 재계산 반올림 차이, 평균 비율 1.00025) → 현행 심볼 SPYM 값 채택

**변경 사항**
- `backend/migrations/0007_splg_to_spym.sql` (신규, 원격 D1 적용 완료 — 4,405행)
  - `ranking_etf` 티커 SPLG → SPYM + 정식 명칭 갱신
  - 죽은 SPLG `price_cache` 4,402건 / `etf_info` 삭제
- `RankingService.js` / `CompareService.js` / `PriceService.js` — KV 캐시 키 빌더를 `export`
- `DailyUpdateService.js` — 위 빌더를 import해 무효화 키 정의 일원화
  - **기존 버그**: 크론이 `compare:{ticker}:...`를 지우는데 실제 키는 `compare:v6:{ticker}:...`라
    매일 compare 캐시 무효화가 전혀 동작하지 않았음 (6h TTL로만 만료)
- 랭킹 캐시 키 `ranking:` → `ranking:v2:` 범프 (SPLG가 섞인 기존 캐시 폐기)

**검증 완료 (운영)**
- SPYM 1Y 수익률 18.94% / 초과수익 +0.08 / 11위 — VOO(18.88%), IVV(18.85%)와 정합
- 최종 종가 2026-09-02 $90.07
- 랭킹에서 SPLG 소멸, 종목 수 30 유지

**참고**: 과거 P-017(2026-03-25)에서 "Yahoo 호환" 사유로 SPYM → SPLG로 되돌린 이력이 있다.
당시엔 Yahoo가 SPYM을 지원하지 않았던 것으로 보이며, 이번에 다시 SPYM으로 정정했다.

### 미해결 (재발 방지 — 사용자 결정 대기)
- 마지막 종가가 5영업일 이상 뒤처진 종목을 **랭킹에서 자동 제외**
- 크론 로그에서 `skipped`를 "정상적으로 새 데이터 없음" / "며칠째 갱신 안 됨(stale)"으로 분리해 경고
- → 티커 변경은 재발 가능성이 높다 (SPYM↔SPLG로 이미 두 차례 발생)
